...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    # Renombrar la columna de tiempo si es necesario
    if 'ts' in df.columns:
...
def main():
    parser = argparse.ArgumentParser(description="Genera features con indicadores técnicos")
    ...
    df_labeled = make_labeled_dataset(df_features, horizon=args.horizon)
    df_labeled.to_csv(args.output, index=False)
    print(f"Archivo de features guardado en {args.output}")
