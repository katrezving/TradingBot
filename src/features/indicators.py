...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        logger.info("Indicadores técnicos generados correctamente.")
        return d
...
def make_labeled_dataset(df_ind: pd.DataFrame, horizon: int = 1) -> pd.DataFrame:
    try:
        ...
        logger.info("Dataset etiquetado generado correctamente.")
        return d
    except Exception as e:
        logger.error(f"Error en make_labeled_dataset: {e}")
        return df_ind
...
def select_feature_cols(df: pd.DataFrame):
    ...
    logger.info(f"Columnas seleccionadas para features: {selected}")
    return selected
